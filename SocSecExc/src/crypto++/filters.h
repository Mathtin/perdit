...
//!   more details.
class CRYPTOPP_DLL CRYPTOPP_NO_VTABLE Filter : public BufferedTransformation,
                                               public NotCopyable {
  public:
    ...
    //! \returns \p true if the object allows attached transformations, \p false
    //! otherwise.
    //! \note Source and Filter offer attached transformations; while Sink does
    //! not.
    bool Attachable() {
        return true;
    }
    ...
    //! \returns pointer to a BufferedTransformation if there is an attached
    //! transformation, \p NULL otherwise.
    ...
    //! \details newAttachment can be a single filter, a chain of filters or \p
    //! NULL.
    //!    Pass \p NULL to remove an existing BufferedTransformation or chain of
    //!    filters
    ...
    size_t TransferTo2(BufferedTransformation &target, lword &transferBytes,
    ...
    // See the documentation for BufferedTransformation in cryptlib.h
    void Initialize(const NameValuePairs &parameters = g_nullNameValuePairs,
                    int propagation = -1);
  ...
  protected:
    ...
    //! \param messageEnd means how many filters to signal MessageEnd() to,
    //! including this one
    //! \param blocking specifies whether the object should block when
    //! processing input
    //! \param channel the channel to process the data
    //! \returns the number of bytes that remain in the block (i.e., bytes not
    //! processed)
    size_t Output(int outputSite, const byte *inString, size_t length,
    ...
    //! \returns the number of bytes that remain in the block (i.e., bytes not
    //! processed)
    size_t OutputModifiable(int outputSite, byte *inString, size_t length,
                            int messageEnd, bool blocking,
                            const std::string &channel = DEFAULT_CHANNEL);

    //! \brief Signals the end of messages to the object
    //! \param outputSite unknown, system crash between keyboard and chair...
    //! \param propagation the number of attached transformations the
    //! MessageEnd() signal should be passed
    //! \param blocking specifies whether the object should block when
    //! processing input
    //! \param channel the channel to process the data
    //! \returns TODO
    ...
    //! <tt>1</tt> means this
    //!   object only. Setting propagation to <tt>-1</tt> means unlimited
    //!   propagation.
    bool OutputMessageEnd(int outputSite, int propagation, bool blocking,
                          const std::string &channel = DEFAULT_CHANNEL);
    ...
    //! output everything, even if
    //!   there may not be enough data to complete the action. For example, hard
    //!   flushing a  HexDecoder
    //!   would cause an error if you do it after inputing an odd number of hex
    //!   encoded characters.
    //! \note For some types of filters, like  ZlibDecompressor, hard flushes
    //! can only
    //!   be done at "synchronization points". These synchronization points are
    //!   positions in the data
    //!   stream that are created by hard flushes on the corresponding reverse
    //!   filters, in this
    //!   example ZlibCompressor. This is useful when zlib compressed data is
    //!   moved across a
    //!   network in packets and compression state is preserved across packets,
    //!   as in the SSH2 protocol.
    bool OutputFlush(int outputSite, bool hardFlush, int propagation,
                     bool blocking,
                     const std::string &channel = DEFAULT_CHANNEL);
    ...
    //! \param outputSite unknown, system crash between keyboard and chair...
    //! \param propagation the number of attached transformations the
    //! MessageSeriesEnd() signal should be passed
    //! \param blocking specifies whether the object should block when
    //! processing input
    //! \param channel the channel to process the data
    //! \returns TODO
    //! \details Each object that receives the signal will perform its
    //! processing, decrement
    //!    propagation, and then pass the signal on to attached transformations
    //!    if the value is not 0.
    //! \details propagation count includes this object. Setting  propagation to
    //! <tt>1</tt> means this
    //!   object only. Setting  propagation to <tt>-1</tt> means unlimited
    //!   propagation.
    //! \note There should be a MessageEnd() immediately before
    //! MessageSeriesEnd().
    bool OutputMessageSeriesEnd(int outputSite, int propagation, bool blocking,
                                const std::string &channel = DEFAULT_CHANNEL);
...
struct CRYPTOPP_DLL FilterPutSpaceHelper {
    ...
    //! \details \p bufferSize is an IN and OUT parameter. If
    //! HelpCreatePutSpace() returns a non-NULL value, then
    //!    bufferSize is valid and provides the size of the working space
    //!    created for the caller.
    //! \details Internally, HelpCreatePutSpace() calls \ref
    //! BufferedTransformation::ChannelCreatePutSpace
    //!   "ChannelCreatePutSpace()" using \p desiredSize. If the target returns
    //!   \p desiredSize with a size less
    //!   than \p minSize (i.e., the request could not be fulfilled), then an
    //!   internal SecByteBlock
    //!   called \p m_tempSpace is resized and used for the caller.
    byte *HelpCreatePutSpace(BufferedTransformation &target,
                             const std::string &channel, size_t minSize,
                             size_t desiredSize, size_t &bufferSize) {
    ...
    //! \param channel channel for the working space
    //! \param minSize minimum size of the allocation, in bytes
    //! \details Internally, the overload calls
    //! HelpCreatePutSpace(BufferedTransformation &target, const std::string
    //! &channel, size_t minSize, size_t desiredSize, size_t &bufferSize) using
    //! \p minSize for missing arguments.
    byte *HelpCreatePutSpace(BufferedTransformation &target,
                             const std::string &channel, size_t minSize) {
    ...
    byte *HelpCreatePutSpace(BufferedTransformation &target,
                             const std::string &channel, size_t minSize,
                             size_t bufferSize) {
        return HelpCreatePutSpace(target, channel, minSize, minSize,
                                  bufferSize);
    }
...
//! \details measure how many bytes and messages pass through the filter. The
//! filter also serves as valve by
...
class CRYPTOPP_DLL MeterFilter : public Bufferless<Filter> {
  public:
    ...
    //! \param attachment an optional attached transformation
    //! \param transparent flag indicating if the filter should function
    //! transparently
    //! \details \p attachment can be \p NULL. The filter is transparent by
    //! default. If the filter is
    //!   transparent, then PutMaybeModifiable() does not process a request and
    //!   always returns 0.
    MeterFilter(BufferedTransformation *attachment = NULL,
    ...
    //! \details Internally, MeterFilter maitains a deque of ranges to skip. As
    //! messages are processed,
    //!   ranges of bytes are skipped according to the list of ranges.
    void AddRangeToSkip(unsigned int message, lword position, lword size,
                        bool sortNow = true);

    //! \brief Resets the meter
    //! \details ResetMeter() reinitializes the meter by setting counters to 0
    //! and removing previous
    //!   skip ranges.
    void ResetMeter();
    ...
    size_t Put2(const byte *inString, size_t length, int messageEnd,
                bool blocking);
    size_t PutModifiable2(byte *inString, size_t length, int messageEnd,
                          bool blocking);
    bool IsolatedMessageSeriesEnd(bool blocking);

  private:
    size_t PutMaybeModifiable(byte *inString, size_t length, int messageEnd,
                              bool blocking, bool modifiable);
    ...
    struct MessageRange {
        inline bool operator<(const MessageRange &b)
            const // BCB2006 workaround: this has to be a member function
        {
...
//! \details FilterWithBufferedInput divides the input stream into a first
//! block, a number of
//!   middle blocks, and a last block. First and last blocks are optional, and
//!   middle blocks may
//!   be a stream instead (i.e. <tt>blockSize == 1</tt>).
//! \sa AuthenticatedEncryptionFilter, AuthenticatedDecryptionFilter,
...
class CRYPTOPP_DLL FilterWithBufferedInput : public Filter {
  public:
    ...
    //! \brief Construct a FilterWithBufferedInput with an attached
    //! transformation
    ...
    //! \param attachment an attached transformation
    //! \details \p firstSize and \p lastSize may be 0. \p blockSize must be at
    //! least 1.
    FilterWithBufferedInput(size_t firstSize, size_t blockSize, size_t lastSize,
                            BufferedTransformation *attachment);

    void IsolatedInitialize(const NameValuePairs &parameters);
    size_t Put2(const byte *inString, size_t length, int messageEnd,
                bool blocking) {
        return PutMaybeModifiable(const_cast<byte *>(inString), length,
                                  messageEnd, blocking, false);
    }
    size_t PutModifiable2(byte *inString, size_t length, int messageEnd,
                          bool blocking) {
    ...
    //! \details The input buffer may contain more than blockSize bytes if
    //! <tt>lastSize != 0</tt>.
  ...
  protected:
    ...
    virtual void
        InitializeDerivedAndReturnNewSizes(const NameValuePairs &parameters,
                                           size_t &firstSize, size_t &blockSize,
                                           size_t &lastSize) {
        CRYPTOPP_UNUSED(parameters);
    ...
    // else if totalLength <= firstSize+lastSize then length ==
    // totalLength-firstSize
    // else lastSize <= length < lastSize+blockSize
    virtual void LastPut(const byte *inString, size_t length) = 0;
    virtual void FlushDerived() {}

  protected:
    size_t PutMaybeModifiable(byte *begin, size_t length, int messageEnd,
                              bool blocking, bool modifiable);
    void NextPutMaybeModifiable(byte *inString, size_t length,
                                bool modifiable) {
        if (modifiable)
    ...
    // This function should no longer be used, put this here to cause a compiler
    // error
...
//! \details FilterWithInputQueue will buffer input using a ByteQueue. When the
//! filter receives
//!   a \ref BufferedTransformation::MessageEnd() "MessageEnd()" signal it will
//!   pass the data
...
struct BlockPaddingSchemeDef {
    ...
    //! \brief Padding schemes used for block ciphers.
    //! \details DEFAULT_PADDING means PKCS_PADDING if
    //! <tt>cipher.MandatoryBlockSize() > 1 &&
    //!   cipher.MinLastBlockSize() == 0</tt>, which holds for ECB or CBC mode.
    //!   Otherwise,
    //!   NO_PADDING for modes like OFB, CFB, CTR, CBC-CTS.
    //! \sa <A HREF="http://www.weidai.com/scan-mirror/csp.html">Block Cipher
    //! Padding</A> for
    //!   additional details.
    enum BlockPaddingScheme {
...
//! \details Filter wrapper for StreamTransformation. The filter will optionally
//! handle padding/unpadding when needed
class CRYPTOPP_DLL StreamTransformationFilter : public FilterWithBufferedInput,
                                                public BlockPaddingSchemeDef,
  ...
  public:
    ...
    //! \param allowAuthenticatedSymmetricCipher flag indicating whether the
    //! filter should allow authenticated encryption schemes
  ...
  protected:
    void InitializeDerivedAndReturnNewSizes(const NameValuePairs &parameters,
                                            size_t &firstSize,
                                            size_t &blockSize,
                                            size_t &lastSize);
    ...
    static size_t LastBlockSize(StreamTransformation &c,
...
class CRYPTOPP_DLL HashFilter : public Bufferless<Filter>,
                                private FilterPutSpaceHelper {
  public:
    //! \brief Construct a HashFilter
    //! \param hm reference to a HashTransformation
    //! \param attachment an optional attached transformation
    //! \param putMessage flag indicating whether the original message should be
    //! passed to an attached transformation
    //! \param truncatedDigestSize the size of the digest
    //! \param messagePutChannel the channel on which the message should be
    //! output
...
class CRYPTOPP_DLL HashVerificationFilter : public FilterWithBufferedInput {
  public:
    ...
    //! \brief Exception thrown when a data integrity check failure is
    //! encountered
    ...
    enum Flags {
        //! \brief Indicates the hash is at the end of the message (i.e.,
        //! concatenation of message+hash)
        HASH_AT_END = 0,
        //! \brief Indicates the hash is at the beginning of the message (i.e.,
        //! concatenation of hash+message)
        HASH_AT_BEGIN = 1,
        //! \brief Indicates the message should be passed to an attached
        //! transformation
        PUT_MESSAGE = 2,
        //! \brief Indicates the hash should be passed to an attached
        //! transformation
        PUT_HASH = 4,
        //! \brief Indicates the result of the verification should be passed to
        //! an attached transformation
        PUT_RESULT = 8,
        //! \brief Indicates the filter should throw a HashVerificationFailed if
    ...
    //! \param truncatedDigestSize the size of the digest
    //! \details <tt>truncatedDigestSize = -1</tt> indicates \ref
    //! HashTransformation::DigestSize() "DigestSize" should be used.
...
//! \details Filter wrapper for encrypting with AuthenticatedSymmetricCipher,
//! optionally handling padding/unpadding when needed
class CRYPTOPP_DLL AuthenticatedEncryptionFilter
  ...
  public:
    ...
    //! \param attachment an optional attached transformation
    //! \param putAAD flag indicating whether the AAD should be passed to an
    ...
    AuthenticatedEncryptionFilter(
        AuthenticatedSymmetricCipher &c,
        BufferedTransformation *attachment = NULL, bool putAAD = false,
        int truncatedDigestSize = -1,
        const std::string &macChannel = DEFAULT_CHANNEL,
        BlockPaddingScheme padding = DEFAULT_PADDING);

    void IsolatedInitialize(const NameValuePairs &parameters);
    byte *ChannelCreatePutSpace(const std::string &channel, size_t &size);
    size_t ChannelPut2(const std::string &channel, const byte *begin,
...
//! \details Filter wrapper wrapper for decrypting with
...
class CRYPTOPP_DLL AuthenticatedDecryptionFilter
  ...
  public:
    ...
    //! \details The flags are a bitmask and can be OR'd together.
    enum Flags {
        //! \brief Indicates the MAC is at the end of the message (i.e.,
        //! concatenation of message+mac)
        MAC_AT_END = 0,
        //! \brief Indicates the MAC is at the beginning of the message (i.e.,
        //! concatenation of mac+message)
        MAC_AT_BEGIN = 1,
        //! \brief Indicates the filter should throw a HashVerificationFailed if
        //! a failure is encountered
        THROW_EXCEPTION = 16,
...
class CRYPTOPP_DLL SignerFilter : public Unflushable<Filter> {
  public:
    ...
    //! \param putMessage flag indicating whether the original message should be
    //! passed to an attached transformation
    SignerFilter(RandomNumberGenerator &rng, const PK_Signer &signer,
                 BufferedTransformation *attachment = NULL,
...
class CRYPTOPP_DLL SignatureVerificationFilter
    : public FilterWithBufferedInput {
  public:
    ...
    //! \details The flags are a bitmask and can be OR'd together.
    enum Flags {
        //! \brief Indicates the signature is at the end of the message (i.e.,
        //! concatenation of message+signature)
        SIGNATURE_AT_END = 0,
        //! \brief Indicates the signature is at the beginning of the message
        //! (i.e., concatenation of signature+message)
        SIGNATURE_AT_BEGIN = 1,
        //! \brief Indicates the message should be passed to an attached
        //! transformation
        PUT_MESSAGE = 2,
        //! \brief Indicates the signature should be passed to an attached
        //! transformation
        PUT_SIGNATURE = 4,
        //! \brief Indicates the result of the verification should be passed to
        //! an attached transformation
    ...
    //! \returns true if the signature on the previosus message was valid, false
    //! otherwise
    bool GetLastResult() const {
        return m_verified;
    }
...
typedef SignatureVerificationFilter
    VerifierFilter; // for backwards compatibility
...
class CRYPTOPP_DLL Redirector : public CustomSignalPropagation<Sink> {
  public:
    ...
    Redirector(BufferedTransformation &target,
    ...
    size_t Put2(const byte *inString, size_t length, int messageEnd,
                bool blocking) {
        return m_target
                   ? m_target->Put2(inString, length,
                                    GetPassSignals() ? messageEnd : 0, blocking)
                   : 0;
    }
    bool Flush(bool hardFlush, int propagation = -1, bool blocking = true) {
        return m_target && GetPassSignals()
                   ? m_target->Flush(hardFlush, propagation, blocking)
                   : false;
    }
    bool MessageSeriesEnd(int propagation = -1, bool blocking = true) {
        return m_target && GetPassSignals()
                   ? m_target->MessageSeriesEnd(propagation, blocking)
                   : false;
    }
    ...
    size_t ChannelPut2(const std::string &channel, const byte *begin,
                       size_t length, int messageEnd, bool blocking) {
        return m_target
                   ? m_target->ChannelPut2(channel, begin, length,
                                           GetPassSignals() ? messageEnd : 0,
                                           blocking)
                   : 0;
    }
    size_t ChannelPutModifiable2(const std::string &channel, byte *begin,
                                 size_t length, int messageEnd, bool blocking) {
        return m_target
                   ? m_target->ChannelPutModifiable2(
                         channel, begin, length,
                         GetPassSignals() ? messageEnd : 0, blocking)
                   : 0;
    }
    bool ChannelFlush(const std::string &channel, bool completeFlush,
                      int propagation = -1, bool blocking = true) {
        return m_target && GetPassSignals()
                   ? m_target->ChannelFlush(channel, completeFlush, propagation,
                                            blocking)
                   : false;
    }
    bool ChannelMessageSeriesEnd(const std::string &channel,
                                 int propagation = -1, bool blocking = true) {
        return m_target && GetPassSignals()
                   ? m_target->ChannelMessageSeriesEnd(channel, propagation,
                                                       blocking)
                   : false;
    }

    unsigned int GetMaxWaitObjectCount() const {
        return m_target && GetPassWaitObjects()
                   ? m_target->GetMaxWaitObjectCount()
                   : 0;
    }
...
class CRYPTOPP_DLL OutputProxy : public CustomSignalPropagation<Sink> {
  public:
    OutputProxy(BufferedTransformation &owner, bool passSignal)
        : m_owner(owner), m_passSignal(passSignal) {}
    ...
    size_t Put2(const byte *inString, size_t length, int messageEnd,
                bool blocking) {
        return m_owner.AttachedTransformation()->Put2(
            inString, length, m_passSignal ? messageEnd : 0, blocking);
    }
    size_t PutModifiable2(byte *begin, size_t length, int messageEnd,
                          bool blocking) {
        return m_owner.AttachedTransformation()->PutModifiable2(
            begin, length, m_passSignal ? messageEnd : 0, blocking);
    }
    void Initialize(const NameValuePairs &parameters = g_nullNameValuePairs,
                    int propagation = -1) {
        if (m_passSignal)
            m_owner.AttachedTransformation()->Initialize(parameters,
                                                         propagation);
    }
    bool Flush(bool hardFlush, int propagation = -1, bool blocking = true) {
        return m_passSignal
                   ? m_owner.AttachedTransformation()->Flush(
                         hardFlush, propagation, blocking)
                   : false;
    }
    bool MessageSeriesEnd(int propagation = -1, bool blocking = true) {
        return m_passSignal
                   ? m_owner.AttachedTransformation()->MessageSeriesEnd(
                         propagation, blocking)
                   : false;
    }

    byte *ChannelCreatePutSpace(const std::string &channel, size_t &size) {
        return m_owner.AttachedTransformation()->ChannelCreatePutSpace(channel,
                                                                       size);
    }
    size_t ChannelPut2(const std::string &channel, const byte *begin,
                       size_t length, int messageEnd, bool blocking) {
        return m_owner.AttachedTransformation()->ChannelPut2(
            channel, begin, length, m_passSignal ? messageEnd : 0, blocking);
    }
    size_t ChannelPutModifiable2(const std::string &channel, byte *begin,
                                 size_t length, int messageEnd, bool blocking) {
    ...
    bool ChannelFlush(const std::string &channel, bool completeFlush,
                      int propagation = -1, bool blocking = true) {
        return m_passSignal
                   ? m_owner.AttachedTransformation()->ChannelFlush(
                         channel, completeFlush, propagation, blocking)
                   : false;
    }
    bool ChannelMessageSeriesEnd(const std::string &channel,
                                 int propagation = -1, bool blocking = true) {
        return m_passSignal
                   ? m_owner.AttachedTransformation()->ChannelMessageSeriesEnd(
                         channel, propagation, blocking)
                   : false;
    }
...
//! \class ProxyFilter
//! \brief Base class for Filter classes that are proxies for a chain of other
//! filters
class CRYPTOPP_DLL ProxyFilter : public FilterWithBufferedInput {
  public:
    ...
    //! \param attachment an attached transformation
    ProxyFilter(BufferedTransformation *filter, size_t firstSize,
                size_t lastSize, BufferedTransformation *attachment);
...
//! \brief Proxy filter that doesn't modify the underlying filter's input or
//! output
class CRYPTOPP_DLL SimpleProxyFilter : public ProxyFilter {
  public:
    ...
    //! \param filter an output filter
    //! \param attachment an attached transformation
    SimpleProxyFilter(BufferedTransformation *filter,
                      BufferedTransformation *attachment)
...
//! \details PK_DecryptorFilter is a proxy for the filter created by
...
class CRYPTOPP_DLL PK_EncryptorFilter : public SimpleProxyFilter {
  public:
    ...
    //! \param attachment an optional attached transformation
    PK_EncryptorFilter(RandomNumberGenerator &rng,
...
class CRYPTOPP_DLL PK_DecryptorFilter : public SimpleProxyFilter {
  public:
    ...
    //! \param attachment an optional attached transformation
    PK_DecryptorFilter(RandomNumberGenerator &rng,
...
//! \details \ref StringSinkTemplate "StringSink" is a StringSinkTemplate
//! typedef
template <class T>
class StringSinkTemplate : public Bufferless<Sink> {
  public:
    ...
    StringSinkTemplate(T &output) : m_output(&output) {
    ...
    size_t Put2(const byte *inString, size_t length, int messageEnd,
        ...
        if (length > 0) {
            ...
            m_output->append((const char_type *)inString,
...
class RandomNumberSink : public Bufferless<Sink> {
  public:
    ...
    RandomNumberSink() : m_rng(NULL) {}

    //! \brief Construct a RandomNumberSink
    //! \param rng a RandomNumberGenerator derived class
    RandomNumberSink(RandomNumberGenerator &rng) : m_rng(&rng) {}
...
class CRYPTOPP_DLL ArraySink : public Bufferless<Sink> {
  public:
    ...
    //! \details Name::OutputBuffer() is a mandatory parameter using this
    //! constructor.
    ArraySink(const NameValuePairs &parameters = g_nullNameValuePairs)
        : m_buf(NULL), m_size(0), m_total(0) {
...
class CRYPTOPP_DLL ArrayXorSink : public ArraySink {
  public:
    ...
    //! \param buf pointer to a memory buffer
    //! \param size length of the memory buffer
    ArrayXorSink(byte *buf, size_t size) : ArraySink(buf, size) {}
...
class StringStore : public Store {
  public:
    ...
    StringStore(const char *string = NULL) {
        StoreInitialize(
            MakeParameters("InputBuffer", ConstByteArrayParameter(string)));
    }
    ...
    StringStore(const byte *string, size_t length) {
        StoreInitialize(MakeParameters(
            "InputBuffer", ConstByteArrayParameter(string, length)));
    }
    ...
    CRYPTOPP_DLL size_t TransferTo2(
        BufferedTransformation &target, lword &transferBytes,
        const std::string &channel = DEFAULT_CHANNEL, bool blocking = true);
    CRYPTOPP_DLL size_t CopyRangeTo2(
...
class CRYPTOPP_DLL RandomNumberStore : public Store {
  public:
    RandomNumberStore() : m_rng(NULL), m_length(0), m_count(0) {}
...
class CRYPTOPP_DLL CRYPTOPP_NO_VTABLE Source : public InputRejecting<Filter> {
  public:
    ...
    //! \note pumpMax is a \p lword, which is a 64-bit value that typically uses
    //! \p LWORD_MAX. The default
    //!   argument is a \p size_t that uses \p SIZE_MAX, and it can be 32-bits
    //!   or 64-bits.
    ...
    //! \returns the number of bytes that remain in the block (i.e., bytes not
    //! processed)
    //! \details byteCount is an \a IN and \a OUT parameter. When the call is
    //! made, byteCount is the
    //!   requested size of the pump. When the call returns, byteCount is the
...
class CRYPTOPP_DLL StringSource : public SourceTemplate<StringStore> {
  public:
    ...
    StringSource(const byte *string, size_t length, bool pumpAll,
        ...
        : SourceTemplate<StringStore>(attachment) {
        SourceInitialize(
            pumpAll, MakeParameters("InputBuffer",
...
//! RNG-based implementation of Source interface
class CRYPTOPP_DLL RandomNumberSource
    : public SourceTemplate<RandomNumberStore> {
  public:
    RandomNumberSource(RandomNumberGenerator &rng, int length, bool pumpAll,
                       BufferedTransformation *attachment = NULL)
